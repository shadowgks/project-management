...
class Menu
{
    ...
    private function _generateItemLink($item)
    {
        ...
        if (isset($item['path'])) {
            ...
            if (isset($item['route']['params'])) {
                $attributes['href'] = route($item['route']['name'], $item['route']['params']);
            } else {
                $attributes['href'] = route($item['route']['name']);
            }
        } else {
            $tag = 'span';
        }
    ...
    private function _generateItemLinkIcon($item)
    {
        ...
        if (isset($item['icon'])) {
            ...
            if ($this->linkLevel === 0 && !empty($this->iconRoot)) {
                ...
                } else if (isset($item['icon']['font'])) {
                    echo '<i class="fa fa-' . $item['icon']['font'] . '"></i>';
                } else {
                    echo $item['icon'];
                }
            }
